...
public struct PathMapping: Identifiable, Hashable {
    public var id: UUID
    public var windowsPrefix: String
    public var macPrefix: String

    public init(id: UUID = UUID(), windowsPrefix: String, macPrefix: String) {
        self.id = id
        self.windowsPrefix = windowsPrefix
        self.macPrefix = macPrefix
    }
}

public struct SharePointMapping: Identifiable, Hashable {
    public var id: UUID
    public var sharePointPrefix: String
    public var localRoot: String

    public init(id: UUID = UUID(), sharePointPrefix: String, localRoot: String) {
        self.id = id
        self.sharePointPrefix = sharePointPrefix
...
public struct PathConverter {
    ...
    public static func makeContext(mappings: [PathMapping], sharePointMappings: [SharePointMapping]) -> ConversionContext {
    ...
    public static func convert(_ input: String, context: ConversionContext) -> String {
    ...
    public static func convert(_ input: String, mappings: [PathMapping] = [], sharePointMappings: [SharePointMapping] = []) -> String {
        convert(input, context: makeContext(mappings: mappings, sharePointMappings: sharePointMappings))
    }

    public static func isWindowsPath(_ input: String) -> Bool {
    ...
    public static func isSharePointURL(_ input: String) -> Bool {
    ...
    private static func windowsToMac(_ input: String, context: ConversionContext) -> String {
        ...
        if input.hasPrefix("\\\\") || input.hasPrefix("//") {
            ...
            let parts = prefixRemoved.split(omittingEmptySubsequences: true, whereSeparator: { $0 == "\\" || $0 == "/" })
            if parts.count >= 2 {
                let server = parts[0]
                let share = parts[1]
    ...
    static func normalizeMappings(_ mappings: [PathMapping]) -> [PathMapping] {
    ...
    static func normalizeSharePointMappings(_ mappings: [SharePointMapping]) -> [SharePointMapping] {
        mappings.map { mapping in
    ...
    static func normalizeHomeRoot(_ root: String) -> String {
